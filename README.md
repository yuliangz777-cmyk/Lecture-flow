...
`.github/workflows/pages.yml` 會把 `static/` 發佈到 GitHub Pages
（只發佈靜態的那一半，Python 後端不會被發佈）。

第一次要手動開啟：

1. repo 需要是 **public**（Pages 在私有 repo 上需要付費方案）。
2. Settings → Pages → Build and deployment → Source 選 **GitHub Actions**。
   這一步 workflow 的 token 沒有權限代做，必須手動點。
3. 到 Actions 分頁手動執行一次 **Deploy the PWA to GitHub Pages**。
4. 想要之後每次 push 都自動發佈，就把 `pages.yml` 裡註解掉的 `push` 觸發加回去。
